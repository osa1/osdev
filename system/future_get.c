...
syscall future_get(future *f, int *i)
{
    ...
    if (f->state == FUTURE_EMPTY)
    {
        ...
        if (f->state != FUTURE_VALID) {
            ...
            /* FUTURE_VALID is actually completely redundant, our semaphore
             * does the same job already */
        }

        /* update the state and read the value */
        /* alternatively, we could just recursively call future_get again.
         * asusming we're running some kind of embedded hardware with very
         * limited memory, I guess this version is better even though contains
